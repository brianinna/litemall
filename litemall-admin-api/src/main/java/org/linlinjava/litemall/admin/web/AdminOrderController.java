...
public class AdminOrderController {
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "查询")
    @GetMapping("/list")
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "统计信息"}, button = "查询")
    @GetMapping("/info")
    public Object info(Integer userId, String orderSn,
                       @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime start,
                       @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime end,
                       @RequestParam(required = false) List<Short> orderStatusArray,
                       @RequestParam(defaultValue = "1") Integer page,
                       ...
                       @Order @RequestParam(defaultValue = "desc") String order) {
        LitemallAdmin current = (LitemallAdmin) SecurityUtils.getSubject().getPrincipal();
        return adminOrderService.info(current.getCid(),userId, orderSn, start, end, orderStatusArray,message, page, limit, sort, order);
    }
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "详情")
    @GetMapping("/detail")
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "订单退款")
    @PostMapping("/refund")
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "订单发货")
    @PostMapping("/ship")
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "订单删除")
    @PostMapping("/delete")
    ...
    @RequiresPermissionsDesc(menu = {"用户管理", "订单管理"}, button = "订单商品回复")
    @PostMapping("/reply")
